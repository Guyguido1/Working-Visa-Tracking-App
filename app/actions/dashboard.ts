...
export async function getDashboardStats(tenantId: number): Promise<DashboardData | ErrorResponse> {
  if (!tenantId || typeof tenantId !== "number") {
    console.error("❌ getDashboardStats: Missing or invalid company_id:", tenantId)
  ...
  try {
    ...
    return {
      success: false,
      name: "Failed to load dashboard data",
    }
  }
}
