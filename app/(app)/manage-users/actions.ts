...
export async function fetchUsers(companyId: number): Promise<User[] | { success: false; error: string }> {
  ...
  try {
    // ✅ TENANT ISOLATION: Only fetch users from the same company
...
export async function fetchCompanyName() {
  try {
    // ✅ TENANT ISOLATION: Get company from session
...
export async function addUser(formData: FormData) {
  ...
  try {
    // ✅ TENANT ISOLATION: Check admin status
    if (!session.is_admin) {
    ...
    // ✅ TENANT ISOLATION: Insert user with session's company_id
    await sql`
      INSERT INTO users (
